...
import { ScanMode, DEMO_ENTRIES, LangEntry } from "./types";
...
export default function UploadZone({
  ...
  return (
    <div className="animate-slide-in-up" style={{ animationDelay: "0.1s" }}>
      ...
      <div
        className="mb-4 border-2 border-dashed border-[#1a3a2a] rounded-lg p-12 text-center cursor-pointer hover:border-[#00ff88]/40 hover:bg-[#00ff88]/3 transition-all duration-300"
      ...
      >
        ...
        <p className="text-xs text-[#2a4a3a] font-['IBM_Plex_Mono']">.bin · .hex · .rom · .dump · любой формат</p>
      </div>

      <div className="flex items-center gap-2 mb-4">
        <div className="h-px flex-1 bg-[#1a2a38]" />
        <span className="text-xs text-[#2a4a5a] font-['IBM_Plex_Mono'] px-2">или попробуйте демо</span>
        <div className="h-px flex-1 bg-[#1a2a38]" />
      </div>
      <button
        onClick={onDemoLoad}
        className="w-full py-2 rounded border border-[#1a3a2a] text-xs text-[#4a6a7a] hover:text-[#00ff88] hover:border-[#00ff88]/30 font-['IBM_Plex_Mono'] transition-colors"
      >
        Загрузить демо-файл → Toyota_ECU_2JZ.bin
      </button>
    </div>
  );
}
